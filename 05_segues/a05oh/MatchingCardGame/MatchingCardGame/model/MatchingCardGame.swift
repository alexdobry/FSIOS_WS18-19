...
final class MatchingCardGame {
    ...
    var scoreIncrease: Int = 5
    
    var scoreDecrease: Int = 3
    
    var delegate: MatchingCardGameDelegate?
//    var delegateSc: HighScoreDelegate?
...
//            delegateSc?.finalScoreWas(with: globalScore)
            
            delegate?.matchingCardGameDidEnd(with: pendingCards, globalScore: globalScore)
        }
    ...
    private func pendingCard(_ pendingCard: Card, isMatchingWith card: Card) -> Bool {
        ...
        if pendingCard.suit == card.suit {
            localScore += scoreIncrease
        }
        
        if pendingCard.rank == card.rank {
            localScore += scoreIncrease * 2
        }
        
        if localScore == 0 {
            localScore -= scoreDecrease
        }
